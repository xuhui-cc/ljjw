...
Page({
  ...
  getSpell: function (str, sp){
    ...
    for (i = 0; i < str.length; i++) {
      if (str.charCodeAt(i) >= 0x4e00) {
        p = that.data.strGB.indexOf(str.charAt(i));
        if (p > -1 && p < 3755) {
          for (t = that.data.qswhSpell.length - 1; t > 0; t = t - 2) if (that.data.qswhSpell[t] <= p) break;
          if (t > 0) ret += that.data.qswhSpell[t - 1] + sp;
        }
      } else {
        ret += str.charAt(i);
      }
  ...
  getdata: function (arr){
    let that = this
    var arr_t = [], dataarr = [], str = [];
    for (var i = 0; i < arr.length; i++){
      arr_t[i] = [that.getSpell(arr[i].realname), arr[i]]
    }
    ...
    for (var i = 0; i < 26; i++){
      ...
      for (var j = 0; j < arr.length; j++){
        ...
        var firstchar = arr_t[j][0].substr(0,1)
        ...
        if (upperchar == firstchar){
          ...
          dataarr[i]['users'].push(arr_t[j][1])
        }
      }
      console.log(dataarr)
      if (flag == false){
